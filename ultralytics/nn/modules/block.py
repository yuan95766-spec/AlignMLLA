...
# class AlignMLLA(nn.Module):
#     """支持双输入的AlignMLLA模块 - 增强数值稳定性"""
    
#     def __init__(self, c2: int, r: int = 16, num_heads: int = 8, num_mlla_layers: int = 2, 
#                  ot_iterations: int = 5, ot_reg: float = 0.5, num_mmd_kernels: int = 3):
#         super().__init__()
#         self.c2 = c2  # 输出通道数
#         self.r = r
...
#         self.num_mlla_layers = num_mlla_layers
#         self.ot_iterations = ot_iterations
#         self.ot_reg = ot_reg  # 增大正则化系数，提高稳定性
...
#         self.mlla_layers = None
#         self.output_proj = None
#         self.norm_layer = None  # 添加归一化层
        
#     def _initialize_layers(self, c1_vis, c1_ir):
#         """动态初始化层以适应输入通道数"""
#         # 通道对齐层（处理不同输入通道数）
#         self.vis_proj = nn.Sequential(
#             nn.Conv2d(c1_vis, self.c2, 1),
#             nn.BatchNorm2d(self.c2),  # 添加BN增加稳定性
#             nn.SiLU()
#         )
#         self.ir_proj = nn.Sequential(
#             nn.Conv2d(c1_ir, self.c2, 1),
#             nn.BatchNorm2d(self.c2),  # 添加BN增加稳定性
#             nn.SiLU()
#         )
        
#         # 用简化版的对齐模块替代复杂的OT和MMD模块
#         # 这可以作为初始训练的稳定版本
...
#             nn.Conv2d(self.c2 * 2, self.c2 * 2, 3, padding=1, groups=2),
#             nn.BatchNorm2d(self.c2 * 2),
#             nn.SiLU(),
#             nn.Conv2d(self.c2 * 2, self.c2, 1),
#             nn.BatchNorm2d(self.c2)
#         )
        
#         # 残差连接的归一化层
#         self.norm_layer = nn.GroupNorm(4, self.c2)
...
#             if len(x) != 2:
#                 raise ValueError(f"AlignMLLA需要2个输入，但得到{len(x)}个")
#             vis_features, ir_features = x
#         else:
#             # 如果是单个张量，尝试分割
#             if len(x.shape) == 3:
#                 x = x.unsqueeze(0)
...
#         if len(ir_features.shape) < 4:
#             ir_features = ir_features.unsqueeze(0)
            
#         # 获取输入通道数并初始化层
#         c1_vis = vis_features.shape[1]
#         c1_ir = ir_features.shape[1]
        
#         if not self.initialized:
#             self._initialize_layers(c1_vis, c1_ir)
#             device = vis_features.device
#             self.to(device)
        
#         # 应用特征投影，确保有相同的通道数
#         vis_aligned = self.vis_proj(vis_features)
#         ir_aligned = self.ir_proj(ir_features)
        
#         # 尺寸对齐
#         if vis_aligned.shape[2:] != ir_aligned.shape[2:]:
#             target_size = vis_aligned.shape[2:]
#             ir_aligned = F.interpolate(ir_aligned, size=target_size, mode='bilinear', align_corners=False)
        
#         # 计算注意力权重 (简化版，不使用复杂的OT或MMD)
#         attention_weights = self.cross_attention(vis_aligned)
        
#         # 特征交互
#         vis_enhanced = vis_aligned * attention_weights
#         ir_enhanced = ir_aligned * (1 - attention_weights)
        
#         # 特征拼接
#         fused_features = torch.cat([vis_enhanced, ir_enhanced], dim=1)
        
#         # 特征融合
#         output = self.fusion_conv(fused_features)
        
#         # 添加残差连接和归一化，提高稳定性
#         output = self.norm_layer(output + vis_aligned)
...
class OptimalTransportAlignment(nn.Module):
    def __init__(self, dim: int, num_iterations: int = 10, reg: float = 0.1):
        super().__init__()
        self.dim = dim
    ...
    def sinkhorn_algorithm(self, cost_matrix: torch.Tensor, reg: float) -> torch.Tensor:
        B, N, M = cost_matrix.shape
        u = torch.zeros(B, N, 1, device=cost_matrix.device)
        v = torch.zeros(B, M, 1, device=cost_matrix.device)
        
        for _ in range(self.num_iterations):
            u = reg * (torch.log(torch.ones(B, N, 1, device=cost_matrix.device) / N) - 
                      torch.logsumexp(-cost_matrix / reg + v.transpose(-2, -1), dim=-1, keepdim=True))
            v = reg * (torch.log(torch.ones(B, M, 1, device=cost_matrix.device) / M) - 
                      torch.logsumexp(-cost_matrix / reg + u, dim=-2, keepdim=True))
        
        transport_matrix = torch.exp((-cost_matrix + u + v.transpose(-2, -1)) / reg)
        return transport_matrix
    
    def forward(self, vis_features: torch.Tensor, ir_features: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        B, C, H, W = vis_features.shape
        
        vis_proj = self.proj_vis(vis_features).view(B, C, -1).transpose(1, 2)
        ir_proj = self.proj_ir(ir_features).view(B, C, -1).transpose(1, 2)
        
        vis_flat = vis_features.view(B, C, -1).transpose(1, 2)
        ir_flat = ir_features.view(B, C, -1).transpose(1, 2)
        
        cost_matrix = torch.cdist(vis_proj, ir_proj, p=2)
        transport_matrix = self.sinkhorn_algorithm(cost_matrix, self.reg)
        
        aligned_vis = torch.bmm(transport_matrix, ir_flat)
        aligned_ir = torch.bmm(transport_matrix.transpose(-2, -1), vis_flat)
        
        aligned_vis = aligned_vis.transpose(1, 2).view(B, C, H, W)
        aligned_ir = aligned_ir.transpose(1, 2).view(B, C, H, W)
        
        return aligned_vis, aligned_ir


class MMDAlignment(nn.Module):
    def __init__(self, dim: int, num_kernels: int = 5, r: int = 16):
        super().__init__()
        self.dim = dim
        self.num_kernels = num_kernels
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        
        self.transform_vis = nn.Sequential(
        ...
        self.transform_ir = nn.Sequential(
            nn.Linear(dim, dim // r, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(dim // r, dim, bias=False),
            nn.Sigmoid()
        ...
        self.kernel_bandwidths = nn.Parameter(torch.logspace(-1, 1, num_kernels))
    ...
    def compute_mmd(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        for bandwidth in self.kernel_bandwidths:
            pairwise_distances = torch.cdist(x, y, p=2) ** 2
            kernel = torch.exp(-pairwise_distances / (2 * bandwidth ** 2))
            
            Kxx = torch.cdist(x, x, p=2) ** 2
            Kxx = torch.exp(-Kxx / (2 * bandwidth ** 2)).mean()
            
            Kyy = torch.cdist(y, y, p=2) ** 2  
            Kyy = torch.exp(-Kyy / (2 * bandwidth ** 2)).mean()
            
            Kxy = kernel.mean()
            
            mmd_loss += Kxx + Kyy - 2 * Kxy
        
        return mmd_loss / self.num_kernels
    
    def forward(self, vis_features: torch.Tensor, ir_features: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        B, C, H, W = vis_features.shape
        
        vis_global = self.avg_pool(vis_features).view(B, C)
        ir_global = self.avg_pool(ir_features).view(B, C)
        
        vis_weights = self.transform_vis(vis_global).view(B, C, 1, 1)
        ir_weights = self.transform_ir(ir_global).view(B, C, 1, 1)
        
        aligned_vis = vis_features * vis_weights
        aligned_ir = ir_features * ir_weights
        
        mmd_loss = self.compute_mmd(vis_global, ir_global)
        
        return aligned_vis, aligned_ir, mmd_loss


class SpatialMLLABlock(nn.Module):
    ...
    def linear_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        ...
        normalizer = torch.einsum('bnc,bc->bn', q, k.sum(dim=1))
