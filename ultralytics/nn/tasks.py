...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, layer in enumerate(d["backbone"] + d["head"]):
        if len(layer) == 4:
            ...
            raise ValueError(f"YAML layer format error at index {i}: {layer}")
        print(f"parse_model: i={i}, f={f}, m={m}")
...
#            print("ch[f]", f, ch[f[0]])
            ...
            if not isinstance(f, (list, tuple)) or len(f) != 2:
                raise ValueError(f"AlignMLLA需要恰好2个输入源，但收到: {f}")
            
            # 获取两个输入的通道数
            c1_vis = ch[f[0]]   # 可见光特征通道数
            c1_ir = ch[f[1]]    # 红外特征通道数
            
            # 检查通道数是否相同（如果不同，模块内部会处理对齐）
            if c1_vis != c1_ir:
                if verbose:
                    LOGGER.warning(f"AlignMLLA输入通道数不匹配: vis={c1_vis}, ir={c1_ir}，将自动对齐")
            
            # 输出通道数 = 第一个参数，如果没有指定则使用第一个输入的通道数
            if len(args) > 0:
                c2 = args[0]
                if c2 != nc:  # 不是分类数
                    c2 = make_divisible(min(c2, max_channels) * width, 8)
            else:
                c2 = c1_vis  # 默认输出通道数等于可见光输入通道数
            
            # 构建参数: [输出通道数, r, num_heads, num_mlla_layers, ...]
            # 保持与原RIFusion相似的参数结构
            if len(args) == 0:
                args = [c2]  # 只有输出通道数
            elif len(args) == 1:
                ...
                args = [c2, *args[1:]]  # 保留其他参数
            
            if verbose:
                LOGGER.info(f"AlignMLLA: vis_ch={c1_vis}, ir_ch={c1_ir}, out_ch={c2}, args={args}")
