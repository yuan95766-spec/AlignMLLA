...
class BaseTrainer:
    ...
    def resume_training(self, ckpt):
        ...
        if ckpt is None or not self.resume:#检查检查点（Checkpoint）是否存在
            return
        best_fitness = 0.0
        start_epoch = ckpt.get("epoch", -1) + 1#设置起始epoch,从检查点中获取epoch值，如果不存在则默认为-1，然后加1作为起始epoch
        #start_epoch = 100
        if ckpt.get("optimizer", None) is not None:#如果检查点中存在优化器状态，则加载优化器状态，并更新best_fitness
