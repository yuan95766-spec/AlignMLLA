...
class yolov8_heatmap:
    ...
    def process(self, img_path,imgir_path, save_path):
        ...
        print(f"获取 grayscale_cam 成功，形状: {grayscale_cam.shape}")

        # =================================================================
        # VVVV 在这里添加下面的代码 VVVV

        # ------------------ 方法1: 直接保存为灰度图 ------------------
        # `grayscale_cam` 的值通常在 0 到 1 之间，需要乘以 255 转换成 0-255 的像素值
        # 然后转换成 8 位无符号整数 (np.uint8)，这是图像的标准格式
        heatmap_gray = np.uint8(255 * grayscale_cam)
        cv2.imwrite('result/debug_grayscale_cam.png', heatmap_gray)
        print("✅ 已将原始灰度热力图保存到 result/debug_grayscale_cam.png")


        # ----------------- 方法2: 保存为彩色热力图 (更直观) -----------------
        # 我们可以给灰度图上色，让它看起来更像传统的热力图
        # cv2.applyColorMap 会应用一个伪彩色映射 (这里用 JET)
        heatmap_color = cv2.applyColorMap(heatmap_gray, cv2.COLORMAP_JET)
        cv2.imwrite('result/debug_color_heatmap.png', heatmap_color)
        print("✅ 已将彩色热力图保存到 result/debug_color_heatmap.png")
        ...
        if self.show_box:
            for data in pred:
                ...
                for det in pred:
                    d = det.cpu().detach().numpy()
                    x1, y1, x2, y2 = d[:4].astype(int)
                    conf = float(d[4])            # 真实置信度
                    cls_id = int(d[5])            # 类别编号
...
def get_params():
    params = {
        'weight': '/home/yuan/my_project/TwoStream_Yolov8-main/TwoStream_Yolov8-main/runs/detect/train6/weights/best.pt', # 现在只需要指定权重即可,不需要指定cfg
        'device': 'cuda:0',
